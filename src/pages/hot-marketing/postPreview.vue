    ...
    export default{
      ...
      route: {
        data (transition) {
          ...
          this.$http.get(config.aHost + 'api/operation/hotMarketingGetData/' + this.activity_id).then((response) => {
            ...
            this.pic_url = response.data.subPrevsee
          }, (response) => {
            // error callback
          })
        }
      ...
      methods: {
        endTip () {
          ...
          _hmt.push(['_trackEvent', _this.$root.statistics + '-activity', _this.$root.statistics + '-activity-send-print-post-expired', 'click'])
        },
        redirectEmailRecord () {
          let _this = this
          _hmt.push(['_trackEvent', _this.$root.statistics + '-activity', _this.$root.statistics + '-activity-send-print-post', 'click'])
        },
        closeDialog () {
