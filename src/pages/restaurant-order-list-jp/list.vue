<template>
  <div class="com-content-box">
    <div @click="calendar" class="date">
      <p>{{dateTime | formatTime "yyyy-MM-dd"}}<i class="arrow"></i></p>
      <!--日历插件-->
    ...
    <div v-if="!no_data" class="total-info-wrap">
      <p class="total-info">計 <span>{{summary_info.summary_count}}</span> 件，<span>{{summary_info.summary_amt | formatCurrency}}</span> 元</p>
      ...
      <li v-for="item in order_list" class="main-box">
        <a class="list-box" v-link="{ path: 'detail'}" @click="orderId(item)">
          <div class="list-title-box font-34">
            <div><p class="color-title">注文番号：{{item.order_sn}}</p></div>
            <div class="text-right"><p class="color-orange font-40">{{item.address}}テーブル</p></div>
          </div>
          <div class="list-content-box">
            <div class="information-box">
              <div class="color-weight font-34"><p>{{item.order_name}}</p></div>
              <div class="font-26 color-light">
                <p>注文時間: {{item.pay_time | formatTime "yyyy-MM-dd hh:mm:ss"}}</p>
              </div>
              <div class="font-26 color-light ">
                <p v-if="item.order_state === 2" class="div-flex order-state-true">決済済み</p>
                <p v-if="item.order_state === 3" class="div-flex order-state-false">取消済み</p>
        ...
        <div class="list-btn">
          <a @click="print(item.order_id)" class="font-30 color-orange">レシート印刷</a>
        </div>
      </li>
    </ul>
  </div>
