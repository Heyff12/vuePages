            ...
            if (!util.checkEmail(this.emailAdress)) {
              ...
              bridge.request({
                ...
                callback: (response) => {
                  ...
                  let userInfo = {
                    userId: res.userid,
                    shopName: encodeURIComponent(res.shopname),
                    mobile: res.mobile
                  }
                  let reqData = Object.assign(data, userInfo)
                  bridge.request({
                    url: config.aHost + 'api/operation/hotMarketingPostMail',
                    method: 'POST',
                    data: reqData,
                    callback: (response) => {
                      this.loadingStatus = false
                      this.visible = true
                      _hmt.push(['_trackEvent', _this.$root.statistics + '-activity', _this.$root.statistics + '-activity-send-print-post-success', 'click'])
                    }
                  })
                }
              })
